...
test('renders crawlable content, metadata, and project links without JavaScript', async ({
  browser,
  baseURL,
}) => {
  ...
  try {
    ...
    await expect(page.locator('meta[name="description"]')).toHaveAttribute(
      'content',
      'Portfolio website for Andrew Mahoney-Fernandes'
    )
    expect(
      await page
        .locator('main section[id]')
        .evaluateAll((sections) => sections.map((section) => section.id))
    ).toEqual(['about', 'skills', 'projects', 'contact'])
...
test('hydrates the interactive islands without an initial projects request', async ({
  page,
}, testInfo) => {
  const errors: string[] = []
  ...
  page.on('request', (request) => {
    if (new URL(request.url()).pathname === '/api/projects') projectRequests++
  })
  await page.goto('/')
  ...
  await expect(page.locator('astro-island[component-export="AboutSection"]')).not.toHaveAttribute(
    'ssr',
    ''
  )
  ...
  await expect(
    page.locator('astro-island[component-export="ProjectsSection"]')
  ).not.toHaveAttribute('ssr', '')
  expect(projectRequests).toBe(0)
  await page.screenshot({ path: testInfo.outputPath('projects.png') })
  await page.locator('#contact').scrollIntoViewIfNeeded()
  await expect(page.locator('astro-island[component-export="ContactSection"]')).not.toHaveAttribute(
    'ssr',
    ''
  )
...
test('refreshes projects and retains the last good cards after a failed refresh', async ({
  page,
}) => {
