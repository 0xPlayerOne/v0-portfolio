  ...
  for (const paddle of paddles) {
    ...
    if (paddle.isVertical) {
      ...
      paddle.targetY = Math.max(
        0,
        Math.min(height - paddle.height, ball.y - paddleCenter - offset),
      );
      ...
      paddle.targetX = Math.max(
        0,
        Math.min(width - paddle.width, ball.x - paddleCenter - offset),
      );
...
>>>>>>> origin/staging
  for (const char of text) {
    const charWidth =
      PIXEL_MAP[char as keyof typeof PIXEL_MAP]?.[0]?.length ?? 0;
