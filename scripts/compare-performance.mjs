...
try {
  ...
  if (values['base-url'] || values['head-url']) {
    if (!values['base-url'] || !values['head-url'])
      throw new Error('Provide both --base-url and --head-url')
    ...
    const fixture = JSON.parse(
      capture('bun', [
        '-e',
        "import {FALLBACK_PINNED_REPOS as a,FALLBACK_POPULAR_REPOS as b,MAX_PROJECTS as n} from './constants/github'; console.log(JSON.stringify([...a,...b].slice(0,n)))",
      ])
    )
    for (const [label, sha] of [
      ...
      if (!source.includes(signature))
        throw new Error(`${label}: project loader changed; update the benchmark fixture injector`)
  ...
  for (let iteration = 1; iteration <= count; iteration++) {
    ...
    for (const label of order)
      samples[label].push(
        await measure(label === 'before' ? beforeUrl : afterUrl, directory, label, iteration)
      )
  }
  ...
  await saveSummary(
    directory,
    { metadata, beforeUrl, afterUrl, samples, before, after, delta },
    markdown
  )
