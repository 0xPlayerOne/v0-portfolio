...
export async function startServer(cwd, port) {
  ...
  const server = spawn('bunx', ['wrangler', 'dev', '--local', '--ip', '127.0.0.1', '--port', String(port)], {
    cwd,
  ...
  let output = ''
  let error
  server.on('error', (cause) => { error = cause })
  server.stdout.on('data', (chunk) => { output = (output + chunk).slice(-24_000) })
  server.stderr.on('data', (chunk) => { output = (output + chunk).slice(-24_000) })
  const close = () => {
...
export async function measure(url, directory, label, iteration) {
  ...
  await command('npx', [
    '--yes', 'lighthouse@13.0.1', url, '--quiet',
    '--chrome-flags=--headless=new --no-sandbox --disable-gpu',
    '--only-categories=performance', '--output=json', `--output-path=${path}`,
  ])
  ...
  const measurements = {
    ...
    javascriptTransferBytes: requests.filter((r) => r.resourceType === 'Script').reduce((total, r) => total + (r.transferSize || 0), 0),
    totalTransferBytes: requests.reduce((total, r) => total + (r.transferSize || 0), 0),
  }
  if (!Object.values(measurements).every(Number.isFinite)) throw new Error('Incomplete Lighthouse metrics')
  return { iteration, measuredAt: lhr.fetchTime, measurements, lighthouseVersion: lhr.lighthouseVersion, userAgent: lhr.userAgent, environment: lhr.environment }
}

export function summarize(samples) {
  if (samples.length === 0) throw new Error('Cannot summarize an empty sample')
  return Object.fromEntries(Object.keys(samples[0].measurements).map((key) => {
...
export function enforce(measurements) {
  ...
  for (const [key, maximum] of Object.entries(budgets.lab)) {
    try {
      console.log(key === 'performanceScore'
