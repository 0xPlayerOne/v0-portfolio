...
function fixture() {
  ...
  const cache = {
    async match(request: RequestInfo | URL) {
      return entries.get(String(request instanceof Request ? request.url : request))?.clone()
    },
...
describe('Cloudflare project cache', () => {
  it('caches a cold request, strips internal headers, and ignores query parameters', async () => {
    ...
    const second = await f.handle(
      new Request('https://andrewmf.com/api/projects?different=2'),
      f.cache,
      f.context
    )
  ...
  it('coalesces concurrent cold requests within an isolate', async () => {
    ...
    const results = await Promise.all(
      Array.from({ length: 5 }, () => f.handle(f.request, f.cache, f.context))
    )
    expect(f.load).toHaveBeenCalledTimes(1)
  ...
  it('does not serve an entry beyond the stale window', async () => {
    ...
    expect((await f.handle(f.request, f.cache, f.context)).headers.get('X-Projects-Cache')).toBe(
      'MISS'
    )
    expect(f.load).toHaveBeenCalledTimes(2)
  })
