...
- Next.js 15.5.18 (App Router) + React 19.2.7
- TypeScript 5.9.3, `strict: true`, path alias `@/*` → `./*`
- Tailwind CSS 3.4.19 + shadcn/ui (Radix primitives) + `lucide-react` icons
...
- Lint/format: ESLint 9 flat config (`next/core-web-vitals` + `next/typescript`), Prettier 3.9 with `prettier-plugin-tailwindcss`
...
- **`next.config.mjs` disables build-time checks**: `eslint.ignoreDuringBuilds: true` AND `typescript.ignoreBuildErrors: true`. `bun run build` will succeed even with lint errors and TS errors. CI is the only enforcement. Always run `bun run lint` and `bun run type:check` locally before considering work done — never trust a green build alone.
...
- **Tailwind dark mode** is via `next-themes` (see `components/theme-provider.tsx`). Color tokens live in `app/globals.css` and `constants/colors.ts`; `tailwind.config.js` references them. Don't sprinkle raw hex values.
